...
def plot_csv(csv_file, x_column, outdir):
    ...
    #data[x_column]=(np.round(np.array(data[x_column]), 2))
    ...
    #sns.lineplot(x=x_column, y='proportion', data=data, errorbar=('ci', ci), err_style='band', marker='o', palette=['red', 'blue', 'green', 'black', 'orange'], hue='migration_matrix', hue_order=['data/rare_migration_prob_matrix.csv', 'data/true_migration_prob_matrix.csv', 'data/moderate_migration_prob_matrix.csv', 'data/high_migration_prob_matrix.csv', 'data/equal_migration_prob_matrix.csv'])
    ...
    #sns.boxplot(x='avg_mutrate', y='value', data=data_melt, color='grey', showfliers=True, fliersize=3, dodge=True)
    ...
    plt.tight_layout()
    plt.savefig(f'{out_dir}scipy_avgMutrate_vs_proportion_lineplot2.png')
    #plt.show()
